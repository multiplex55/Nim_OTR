...
- With **Mouse Crop** enabled, click-drag on the overlay to draw a rubber-band rectangle (even if the crop window is not foregrounded); release to apply the crop and sync the dialog values. Drags stay inside the overlay bounds, require a minimal size, and can be cancelled with `Esc`; very small drags are ignored. The overlay status text reflects when mouse crop is armed/dragging.
